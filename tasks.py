...
http://{public_fqdn}/ >> {override_fn}".format(**envs), pty=True)
    ...
    try:
        current_allowed = ast.literal_eval(os.getenv('ALLOWED_HOSTS') or \
                                           "['{public_fqdn}', '{public_host}', 'localhost', 'django', 'coat_project',]".format(**envs))
    except ValueError:
        current_allowed = []
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'coat_project.settings')
    return settings
...
def _geonode_public_port():
    ...
    if not gn_pub_port:
        gn_pub_port = _container_exposed_port(
            'nginx',
            os.getenv('GEONODE_INSTANCE_NAME', 'coat_project')
        )
    return gn_pub_port
