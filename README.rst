...
Since this application uses geonode, base source of settings is ``geonode.settings`` module. It provides defaults for many items, which are used by geonode. This application has own settings module, ``coat_project.settings``, which includes ``geonode.settings``. It customizes few elements:
 ...
 * Adds ``coat_project`` to installed applications, updates templates, staticfiles dirs, sets urlconf to ``coat_project.urls``.

Whether you deploy development or production environment, you should create additional settings file. Convention is to make ``coat_project.local_settings`` module. It is recommended to use ``coat_project/local_settings.py``.. That file contains small subset of settings for edition. It should:
...
::
    ...
    |   geonode.settings     |     ->      |  coat_project.settings    |      ->     |  coat_project.local_settings |
    +------------------------|-------------|-------------------------------|-------------|----------------------------------+
