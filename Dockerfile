...
RUN mkdir -p /usr/src/coat_project

WORKDIR /usr/src/coat_project
...
COPY . /usr/src/coat_project

RUN chmod +x /usr/src/coat_project/tasks.py \
    && chmod +x /usr/src/coat_project/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/coat_project/entrypoint.sh"]